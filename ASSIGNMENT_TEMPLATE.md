...
|-------------|------------|
| Main Predicate | `{{MAIN_PREDICATE}}` |
| Minimum Facts | {{MIN_FACTS}} |
| Minimum Rules | {{MIN_RULES}} |
| Minimum Recursive Predicates | {{MIN_RECURSIVE}} |
...
- At least **{{MIN_FACTS}} facts** representing domain knowledge
- Facts should be well-organized and documented
- Use meaningful predicate and constant names

### 2. Inference Rules (`src/rules.pl`)
- At least **{{MIN_RULES}} rules** for logical inference
- At least **{{MIN_RECURSIVE}} recursive predicates**
- Main query predicate: `{{MAIN_PREDICATE}}`
